...
extern unsigned long __weak get_gpumem_by_pid(pid_t pid, int mem_type);
unsigned long __weak get_gpumem_by_pid(pid_t pid, int mem_type)
{
	return 0;
}
...
void mm_get_all_pmem(int *cnt, struct process_mem *val)
{
	...
	mutex_lock(&mm_mutex);
	*cnt = record_tasks;
	memcpy(val, pmem, sizeof(struct process_mem) * record_tasks);
	mutex_unlock(&mm_mutex);
}
